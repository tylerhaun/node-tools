...
const debug = requre("debug")(__filename);

var template = "{{timestampH}} {{request.ip}} {{request.method}} {{request.path}} {{response.status}} {{response.errorMessage}} {{response.elapsedTime}}"
...
class RequestLogger {
    ...
    getElapsedTime() {
        ...
        var ms = (this.response._hrtime[0] - this.request._hrtime[0]) * 1e3 +
        (this.response._hrtime[1] - this.request._hrtime[1]) * 1e-6
...
export function requestLoggerWrapper(options) {
